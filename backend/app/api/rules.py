...
def _require_rule_write_access(request: Request, rule: dict):
    ...
    # private rule — creator or org owner may modify it
    if scope == "private":
        creator_prefix = rule.get("creator_key_prefix")
        # Creator can always modify their own rule
        ...
        if creator_prefix:
            # Known creator: allow org owner of the creator's org
            creator_org = db_get_org_by_key_prefix(creator_prefix)
            if creator_org and is_org_owner(creator_org, ctx["key_prefix"]):
                return
        else:
            # Legacy rule (no creator tracked): allow org owner of the caller's org
            caller_org = ctx.get("org_id")
            if caller_org and is_org_owner(caller_org, ctx["key_prefix"]):
                return
        raise HTTPException(status_code=403,
