...
def db_add_key(key_hash: str, key_prefix: str, name: str, role: str = "user",
               ...
               key_plain: Optional[str] = None) -> Dict[str, Any]:
    """Insert a new hashed API key into the DB and return its record."""
    conn = _get_conn()
    conn.execute(
        """
        INSERT INTO api_keys (key_hash, key_prefix, key_plain, name, role, org_id, enabled, expires_at)
        VALUES (?, ?, ?, ?, ?, ?, 1, ?)
        """,
        (key_hash, key_prefix, key_plain, name, role, org_id, expires_at),
    )
    conn.commit()
    row = conn.execute(
...
def db_get_all_keys() -> List[Dict[str, Any]]:
    ...
    conn = _get_conn()
    rows = conn.execute(
        "SELECT id, key_prefix, name, role, org_id, enabled, created_at, expires_at FROM api_keys ORDER BY id"
    ).fetchall()
